...
rule all:
    input:
        expand("../infant/data/reads/processed/paired_{sample}_{num}.fastq", sample=sample_list, num=[1,2])

# Download the files from the SRA using fasterq-dump
# clear cache following download of sample
rule sra_download:
    output:
        temp("../infant/data/reads/raw/{sample}_1.fastq"),
        temp("../infant/data/reads/raw/{sample}_2.fastq")
    params:
        fasterq=config["fasterq"],
        cache=config["cache"],
        samplename="{sample}",
        outdir="../infant/data/reads/raw/"
    threads: 20
...
rule trimmomatic:
    input:
        fwd="../infant/data/reads/raw/{sample}_1.fastq",
        rev="../infant/data/reads/raw/{sample}_2.fastq"
    output:
        fwd_pair="../infant/data/reads/processed/paired_{sample}_1.fastq",
        fwd_unpair="../infant/data/reads/processed/unpaired_{sample}_1.fastq",
        rev_pair="../infant/data/reads/processed/paired_{sample}_2.fastq",
        rev_unpair="../infant/data/reads/processed/unpaired_{sample}_2.fastq"
    threads: 20
    params:
