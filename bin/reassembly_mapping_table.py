...
for file in glob.glob("../reassembly_mapping_data/sorted*.txt"):
    sample_name=os.path.basename(file).split(".")[0].split("_")[2]
    with open(file) as fh:
        for line in fh:
            master_dict[sample_name][line.split("\t")[0]]=line.split("\t")[2]

df=pandas.DataFrame(master_dict)
df.to_csv("../reassembly_mapping_data/aggregated.tsv", sep="\t", na_rep="0")
